...
| `log`    | Logs, as normal.           |
| `redact` | Degrades to logging.       |
| `mask`   | Degrades to logging.       |
...
The `degraded_from` key appears only when the run continued despite a configured action that could not be applied. A blocked run never reports it.
